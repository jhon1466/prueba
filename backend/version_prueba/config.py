...
import requests
import yaml
from version_prueba.apps.webui.internal.db import Base, get_db
...
def run_migrations():
    ...
    try:
        ...
        alembic_cfg = Config(version_prueba_DIR / "alembic.ini")

        # Set the script location dynamically
        migrations_path = version_prueba_DIR / "migrations"
...
STATIC_DIR = Path(os.getenv("STATIC_DIR", version_prueba_DIR / "static")).resolve()
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (Version Prueba + Ollama)
            # with the docker build arg USE_OLLAMA=true (--build-arg="USE_OLLAMA=true") this only works with http://localhost:11434
            OLLAMA_BASE_URL = "http://localhost:11434"
        else:
            OLLAMA_BASE_URL = "http://host.docker.internal:11434"
    elif K8S_FLAG:
        OLLAMA_BASE_URL = "http://ollama-service.version-prueba.svc.cluster.local:11434"
