...
from version_prueba.env import ENV, SRC_LOG_LEVELS
...
from version_prueba.utils.payload import (
...
async def verify_connection(
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Version Prueba: Server Connection Error"
            )
...
async def post_streaming_url(
    ...
    try:
        ...
        error_detail = "Version Prueba: Server Connection Error"
        if r is not None:
            try:
                res = await r.json()
                if "error" in res:
